...
def new_reservation():
    ...
    if form.validate_on_submit():
        ...
        user = User.query.filter_by(email=str(current_user.email)).first()
        reservation = Reservation(package=form.package.data, date=form.date.data, location=form.location.data, occasion=form.occasion.data, addons=form.addons.data, user_id=user.id)
